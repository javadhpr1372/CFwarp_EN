...
wpygV="22.9.27 V 0.3"
remoteV=`wget -qO- https://gitlab.com/en_project/cfwarp_en/-/raw/main/CFwarp.sh | sed -n 4p | cut -d '"' -f 2`
red='\033[0;31m'
bblue='\033[0;34m'
yellow='\033[0;33m'
green='\033[0;32m'
...
case ${wgcfv4} in
plus)
WARPIPv4Status=$(white "WARP+status:\c" ; rred "Running, $cfplus" ; white "Service provider Cloudflare obtains IPV4 address:\c" ; rred "$v4 $country" ; white "Netflix NF unlocking status :\c" ; rred "$(./nf | awk '{print $1}' | sed -n '3p')");;  
on)
...
off)
WARPIPv4Status=$(white "WARP status:\c" ; yellow "Closing" ; white "Service provider $isp4 obtains IPV4 address:\c" ; yellow "$v4 $country" ; white "Netflix NF unlocking status:\ c" ; yellow "$(./nf | awk '{print $1}' | sed -n '3p')");;
esac
else
...
case ${wgcfv6} in
plus)
WARPIPv6Status=$(white "WARP+status:\c" ; rred "Running, $cfplus" ; white "Service provider Cloudflare obtains IPV6 address:\c" ; rred "$v6 $country" ; white "Netflix NF unlocking status :\c" ; rred "$(./nf | awk '{print $1}' | sed -n '7p')");;  
on)
...
off)
WARPIPv6Status=$(white "WARP status:\c" ; yellow "closed" ; white "service provider $isp6 obtains IPV6 address:\c" ; yellow "$v6 $country" ; white "Netflix NF unlocking status:\ c" ; yellow "$(./nf | awk '{print $1}' | sed -n '7p')");;
esac
else
...
white "To exit the script, press Ctrl+C"
...
green "Failed to install WARP, suggest the following:"
...
EOF
readp "When the warp state is running, re-detect the warp state interval time (enter the default 60 seconds), please enter the interval time (for example: 50 seconds, enter 50):" stop
[[ -n $stop ]] && sed -i "s/60s/${stop}s/g;s/60s/${stop}s/g" /root/WARP-UP.sh || green " The default interval is 60 seconds"
readp "When the warp state is interrupted (5 consecutive failures to automatically close the warp), continue to detect the interval time of the WARP state (the default is 50 seconds if you press enter), please enter the interval time (for example: 50 seconds, enter 50):" goon
...
readp "$ab" cd
case "$cd" in     
# 1 ) [[ $(type -P wg-quick) ]] && (cwg ; $wj ; green "Wgcf-WARP(+) uninstall complete" && ShowWGCF && WGCFmenu && back) || (yellow "Wgcf is not installed -WARP(+), cannot uninstall " && bash CFwarp.sh);;
...
1 ) [[ ! $(type -P wg-quick) && ! $(type -P warp-cli) ]] && (red "does not install any warp function, cannot uninstall" && CFwarp.sh) || ( cron3 ; cron2 ; $cron1 ; cwg ; $wj ; green "All warps have been uninstalled" && ShowWGCF && WGCFmenu && exit);;
0 ) WARPOC
esac
}
...
green "Current CFwarp script version number: ${wpygV} reset version 2, already the latest version\n"
else
